...
export default class FacePile extends PureComponent {
	...
	_renderOverflowCircle = (overflow) => {
		const { circleStyle, overflowStyle, overflowLabelStyle, circleSize, offset } = this.props;
	...
	render() {
		...
		return (
			<View style={[ styles.container, containerStyle ]}>
				...
				{this.props.isAddNew && (
					<View style={styles.addNewStudent}>
						<TouchableOpacity
							style={_styles.addButton}
							onPress={() => this.props.enrollStudentHandler(true)}
						>
							<Icons.MaterialIcons name="add" color="white" size={20} />
						</TouchableOpacity>
					</View>
				)}
			</View>
