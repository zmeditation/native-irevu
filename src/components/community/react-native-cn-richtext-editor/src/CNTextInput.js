...
class CNTextInput extends Component {
	...
	onSelectionChange = (event) => {
		...
		if (
			(this.justToolAdded == true && selection.start == selection.end && selection.end >= this.textLength) ||
	...
	addTextToContent(content, cursorPosition, textToAdd) {
		...
		if (content[foundIndex].readOnly === true) {
			if (content[foundIndex].text.length === foundItemNo) {
				if (
					...
					foundIndex += 1;
					foundItemNo = 0;
				} else if (this.upComingStype && this.upComingStype.sel.end === cursorPosition) {
				} else {
		...
		if (
			...
			if (
				...
				!(foundIndex === content.length - 1 && foundItemNo === content[foundIndex].len)
			) {
				this.justToolAdded = true;
			}
		}
	...
	removeTextFromContent(content, cursorPosition, removeLength) {
		...
		for (let i = 0; i < removeIndexes.length; i++) {
			...
			if (
				remIndex == 0 &&
				(content.length == 1 || (content.length > 1 && content[1].NewLine == true && content[0].len == 0))
			) {
				content[0].text = '';
				content[0].len = 0;
	...
	addToUpComming(toolType) {
		if (this.upComingStype) {
			...
			const newUpStype = this.upComingStype
				? indexOfUpToolType != -1
					? update(this.upComingStype.stype, { $splice: [ [ indexOfUpToolType, 1 ] ] })
	...
	handleKeyDown = (e) => {
		...
		if (e.nativeEvent.key === 'Backspace' && this.state.selection.start === 0 && this.state.selection.end === 0) {
	...
	render() {
		...
		return (
			...
			>
				{_.map(items, (item) => <CNStyledText key={item.id} style={item.styleList} text={item.text} />)}
			</TextInput>
		);
	}
